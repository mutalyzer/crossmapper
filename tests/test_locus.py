...
from mutalyzer_crossmapper.locus import Point, Coord
...
from mutalyzer_crossmapper.locus import Locus, Coord, Point as LocusPoint

## Test Locus model and its point model
...
def test_Locus_invalid_point():
    """Forward orientent Locus with invalid point."""
    locus = Locus((30, 35))
    with pytest.raises(ValueError):
        locus.to_coordinate(LocusPoint(position=-5, offset=0))
    with pytest.raises(IndexError):
        locus.to_coordinate(LocusPoint(position=5, offset=0))
    with pytest.raises(IndexError):
        locus.to_coordinate(LocusPoint(position=0, offset=2))
    with pytest.raises(IndexError):
        locus.to_coordinate(LocusPoint(position=4, offset=-2))
    with pytest.raises(ValueError):
        locus.to_coordinate(LocusPoint(position=2, offset=1))



def test_Locus_inverted_invalid_point():
